...
<c:url var="addAction" value="/jobHistory/add" ></c:url>

<form:form action="${addAction}" commandName="jobHistory">
<table>
	<c:if test="${!empty jobHistory.name}">
	<tr>
		<td>
			<form:label path="id">
	...
	<tr>
		<td colspan="2">
			<c:if test="${!empty jobHistory.name}">
				<input type="submit"
					value="<spring:message text="Edit Job History"/>" />
			</c:if>
			<c:if test="${empty jobHistory.name}">
				<input type="submit"
					value="<spring:message text="Add Job History"/>" />
			</c:if>
		</td>
	</tr>
</table>	
